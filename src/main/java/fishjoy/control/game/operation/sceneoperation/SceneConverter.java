		...
		Sprite nextButton = new Sprite(CAMERA_WIDTH * 0.5f + 100, CAMERA_HEIGHT * 0.5f, nextTextureRegion) {
			...
			public boolean onAreaTouched(final TouchEvent pSceneTouchEvent, final float pTouchAreaLocalX, final float pTouchAreaLocalY) {
				...
				if(!isEstablished)
				{
					...
					dialog.setView(view);
					dialog.setPositiveButton("»∑ÔøΩÔøΩ", new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog, int which) {
							//unimplemented method
							EditText editText=(EditText)view.findViewById(R.id.edittext);
							if(editText.getText().toString().equals(""))
							{
								Toast.makeText(activity, "ÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩ≈", Toast.LENGTH_SHORT).show();
								dialog.cancel();
							}
							else
							{
								...
								Toast.makeText(activity, "–ª–ª", Toast.LENGTH_SHORT).show();
								dialog.dismiss();
								mDataBase.close();
							}
						}});

					dialog.setNegativeButton("»°ÔøΩÔøΩ", new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog, int which) {
							//unimplemented method
							dialog.dismiss();
						}});
	...
	public void onGameEnd(int mScore, int minRecord, Font mWinFont, Font mLoseFont, Font mScoreFont, Font mTimeFont,
		...
		if (mScore >= Max)
		{
			if(!isEstablished){
				...
				Text inputText = new Text(0, 0, mTimeFont, "ÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩ¬µƒºÔøΩ¬ºÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩƒ¥ÔøΩÔøΩÔø", HorizontalAlign.CENTER);
